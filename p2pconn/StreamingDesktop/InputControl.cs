...
namespace p2pconn
{
    public class InputControl : System.MarshalByRefObject
    {
        ...
        public static void PressMouseButton(bool Left, int X, int Y)
        {
            try
            {
                INPUT mouseDownInput = new INPUT();
                mouseDownInput.type = SendInputEventType.InputMouse;
                mouseDownInput.mkhi.mi.dwFlags = Left ? MouseEventFlags.MOUSEEVENTF_LEFTDOWN : MouseEventFlags.MOUSEEVENTF_RIGHTDOWN;
                SendInput(1, ref mouseDownInput, Marshal.SizeOf(new INPUT()));
            }
            catch { }
        }

        public static void ReleaseMouseButton(bool Left, int X, int Y)
        {
            try
            {
                INPUT mouseUpInput = new INPUT();
                mouseUpInput.type = SendInputEventType.InputMouse;
                mouseUpInput.mkhi.mi.dwFlags = Left ? MouseEventFlags.MOUSEEVENTF_LEFTUP : MouseEventFlags.MOUSEEVENTF_RIGHTUP;
                SendInput(1, ref mouseUpInput, Marshal.SizeOf(new INPUT()));
            }
            catch { }
        }

        public void SendKeyDown(byte VirtualKeyCode)
        {
            try
            {
                INPUT KeyboardInput = new INPUT();
        ...
        public void SendKeyUp(byte VirtualKeyCode)
        {
            try
            {
                INPUT KeyboardInput = new INPUT();
                ...
                KeyboardInput.mkhi.ki.dwExtraInfo = 0;
                KeyboardInput.mkhi.ki.time = 0;
                KeyboardInput.mkhi.ki.dwFlags = KEYEVENTF_KEYUP;
                SendInput(1, ref KeyboardInput, Marshal.SizeOf(new INPUT()));
            }
            catch { }
        }
